                    ...
                    if request.POST.get(weektime) == 'on':
                        ...
                        if oldpeople:
                            if  name not in oldpeople[0].name:
                                ON_DUTY.objects.update_or_create(time=weektime, defaults={'name':name+','+oldpeople[0].name})
                        else:
...
def lend(request):
    ...
    if book.is_lend==False:
        Book.objects.filter(id=request.GET.get('id')).update(lend_people_id=request.session['id'],lend_time=datetime.datetime.now(),is_lend=True)
